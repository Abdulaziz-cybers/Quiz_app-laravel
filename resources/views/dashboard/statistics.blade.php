                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        @foreach($data as $item)
                            <tr>
                                <td class="px-6 py-4 text-sm text-gray-900"><a href="{{ route('results', ['slug'=>$item['result']->quiz->slug]) }}">{{ $item['result']->quiz->title }}</a></td>
                                <td class="px-6 py-4 text-sm text-gray-500"><a href="{{ route('results', ['slug'=>$item['result']->quiz->slug]) }}">{{ $item['result']->started_at }}</a></td>
                                <td class="px-6 py-4 text-sm text-gray-900"><a href="{{ route('results', ['slug'=>$item['result']->quiz->slug]) }}">{{ $item['score'] }}%</a></td>
                                <td class="px-6 py-4 text-sm text-gray-500"><a href="{{ route('results', ['slug'=>$item['result']->quiz->slug]) }}">{{ $item['time_taken'] }}</a></td>
                                <td class="px-6 py-4">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800"><a
                                            href="{{ route('results', ['result'=>$item['result']->id]) }}">{{ $item['status'] }}</a></span>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
