document.addEventListener('DOMContentLoaded', function() {
    let questionCounter = document.querySelectorAll('[data-question-id]').length;
    ...
    function createOptionElement(questionNum, optionNum) {
        ...
        div.innerHTML = `
            <input type="radio" name="questions[${questionNum}][correct]" value="${optionNum - 1}" class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500">
            <input type="text" name="questions[${questionNum}][options][]" placeholder="Option ${optionNum}" required
                class="w-full px-4 py-2 border rounded-lg block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
            <button type="button" class="removeOptionBtn px-2 py-1 text-red-600 hover:text-red-800">×</button>
        `;
        return div;
    ...
    function updateQuestionIndexes() {
        ...
        questions.forEach((question, index) => {
            ...
            questionInput.name = `questions[${questionNum}][quiz]`;

            // Update radio button names
            const radioInputs = question.querySelectorAll('input[type="radio"]');
            radioInputs.forEach((radio, optionIndex) => {
                radio.name = `questions[${questionNum}][correct]`;
            ...
            optionInputs.forEach(option => {
                option.name = `questions[${questionNum}][options][]`;
            });
        });
    }
});
